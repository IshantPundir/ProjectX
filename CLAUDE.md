...
| 3D.report | **Post-session report** (`app/modules/reporting/`) — 3-layer hybrid scorer over the engine's append-only `SessionEvidence`: per-question grade vs the bank rubric → roll-up to per-signal → LLM narrative. Verdict-driven fit-score (Overall/Technical/Behavioral/Communication → tier → verdict; ceiling-capped; Borderline always human-held). `session_reports` table; recruiter report viewer + ReviewTheater playback (R2 recording, presigned). Prompts under `prompts/v4/report_scorer/`. | ✅ done |
| 3D.reel | **Candidate Reel** (`app/modules/reel/`) — ~45–60s AI-directed highlight video. Director LLM (`gpt-5.4`) builds an EDL from report ground-truth + the word-timed `SessionEvidence` transcript; clips cut from the R2 recording via the engine word/turn timing mapped to video by a recording offset (`meta.started_at − recording_started_at`); branded cards + Arjun TTS narration + burned captions; rendered in the `nexus-vision-worker` (ffmpeg). `session_reels` table; eligibility gated to advance/borderline verdicts. Prompts under `prompts/v3/reel/`. | ✅ done |
