...
- **Schema management:** Supabase SQL for the initial cut + Supabase-managed objects (auth hook, `supabase_auth_admin` grants); Alembic for every incremental change after that. `migrations/versions/` head is `0046_reset_emptied_banks` (recent: `0042` per-question difficulty, `0043` session proctoring, and the v2 chain `0044` `job_postings.interview_engine_version` → `0045` bank spoken fields/`primary_signal` → `0046` bank reset).
...
- **Phase 3D.engine (v1, legacy)** — `app/modules/interview_engine/` is a thin LiveKit `AgentSession` harness: `agent.py` runs an STT → LLM → TTS loop with a system prompt built from the JD + question bank (no structured per-question scoring). It is the **global default** (`INTERVIEW_ENGINE_DEFAULT_VERSION=v1`) and dev fallback; the conversation/decision core has been rewritten as **v2** (below), which is the engine under active development. `event_log/` writes a per-session audit envelope to `engine-events/<session_id>.json`.
- **Phase 3D.engine-v2 — the rewrite, MERGED to `main` 2026-05-24, default-OFF (per-job opt-in).** `app/modules/interview_engine_v2/` is a parallel, structured engine built as a **three-tier conversation/control split** (the response to the v1 perf/quality rework). The legacy `interview_engine/` is reference-only and selected only as the dev fallback.
  - **triage** (`triage/`, `gpt-5.4-mini`, ~1–1.5s) — classifies the committed turn (answering / clarification / job-question / no-experience / wants-to-end / …) and SPEAKS the immediate human beat (a filler, a continuation cue, or a verbatim repeat). It judges TURN-completeness (did they finish speaking?), never answer quality, and never sees the rubric.
  - **brain** (`brain/`, `gpt-5.4-mini`, run **async/parallel**, masked by triage's beat, never on the critical path) — the control plane: grades the answer vs the active question's rubric, tracks signal `coverage` (`coverage.py`), runs deterministic policy gates (`brain/policy.py`: verified-knockout, no-leak scrub, grade↔move coherence), and emits exactly one `Directive` from a closed move set (advance / probe / clarify / confirm / redirect / hold / answer_meta / knockout_close / close).
  - **mouth** (`mouth/`, `gpt-5.4-mini`, persona "Arjun") — renders the directive as natural spoken Indian English. NEVER sees the rubric (no-leak by construction); candidate speech is fenced as DATA (identity lock).
  - Supporting: `turn_taking/{floor,eou,pacing}.py` (livekit-free floor-control substrate), `directive.py` (closed act/move vocabulary + `is_terminal`), `event_log/` + `result_builder.py` (audit envelope + `SessionResult` with `coverage_summary`), `selection.py` (`should_run_v2`).
  - **Selection / cutover:** per-job `job_postings.interview_engine_version` (migration `0044`) `??` global `INTERVIEW_ENGINE_DEFAULT_VERSION` (default `v1`), resolved onto `SessionConfig.interview_engine_version` in `build_session_config`; `interview_engine/agent.py`'s entrypoint branches on `should_run_v2`. Default-OFF, so the merge changed no production behavior.
  - **Prompts** live under `prompts/v3/engine/` (`brain.system.txt`, `triage.system.txt`, `mouth/*`), read fresh per-session by `PromptLoader`. **Real-API prompt-evals** in `tests/interview_engine_v2/prompt_evals/` (`@prompt_quality`, opt-in: `pytest -m prompt_quality`) — the standing rule is to validate prompt behavior against the real OpenAI endpoint, not just mocks.

  Schema surface **consumed by v2** (v1 leaves it dormant):
  - `stage_questions.question_kind` + `primary_signal` + per-question `difficulty` — v2's bank taxonomy, signal-coverage map, and grading strictness; the bank-generator emits them.
  - `tenant_settings.engine_agent_name` — read at session start (v2 persona falls back to it; `engine_mouth_persona_name` default "Arjun"). `engine_knockout_policy` still unread.
  - `sessions.knockout_failures` JSONB + `KnockoutFailure` model — v2 still writes `[]`; a knockout currently lands in `coverage_summary` (signal=`failed`) + the audit trail, not yet the typed list (documented follow-up).
...
│       ├── interview_engine/     ← Phase 3D engine v1 (legacy, global default): thin LiveKit AgentSession harness (agent.py); entrypoint branches to v2 via selection.should_run_v2; event_log/ → engine-events/<id>.json
│       ├── interview_engine_v2/  ← Phase 3D engine v2 (the rewrite, default-OFF per-job): triage/ ∥ brain/ → mouth/ ; coverage.py, turn_taking/{floor,eou,pacing}.py, directive.py, policy gates, event_log/ + result_builder.py, selection.py
...
│   └── v3/engine/                ← engine-v2 prompts: brain.system.txt, triage.system.txt, mouth/* (per-act)
├── migrations/                  ← Alembic — head is `0046_reset_emptied_banks`
│   └── versions/
├── tests/
...
- `app/ai/realtime.py` is the second blessed import site for vendor SDKs. It owns LiveKit plugin instantiation (`livekit.plugins.openai`, `deepgram`, `cartesia`, `sarvam`, `ai_coustics`, `turn_detector`) so the interview-engine entrypoint never touches them directly. Reads model IDs / voices / effort from `AIConfig` — never from env or settings. (`interview_agent_name` is the only LiveKit fleet-wide routing label that lives in `settings`; AIConfig is for AI provider config only.) Lazy imports inside each factory keep the FastAPI nexus process free of the realtime plugin packages even though they're installed in the same image — the engine compose service is the only one that loads them. Current factories: `build_noise_cancellation()` (ai_coustics QUAIL_L or QUAIL_VF, env-selected), `build_interruption_options()` (`mode="adaptive"` with `min_words=2`), `build_vad()` (returns `ai_coustics.VAD()`), `build_stt_plugin()` (provider dispatcher: `deepgram` default — `nova-3`, `en-IN`, + per-session keyterm boosting — with `sarvam` as the alternate), `build_tts_plugin()` (provider dispatcher: `sarvam` default — `bulbul:v3` — with `openai` / `cartesia` alternates).
...
- The engine runs as the `nexus-engine` compose service from the **same Docker image** as `nexus` and `nexus-worker`, just with command `python -m app.modules.interview_engine` (this entrypoint dispatches to v1 or v2 per `selection.should_run_v2(SessionConfig)`). Single venv. The two-venv layout that existed pre-Phase-3 was retired when the engine source tree merged into nexus. After editing v2 prompts under `prompts/v3/engine/`, restart with `docker compose up -d --force-recreate nexus-engine` (a per-session `PromptLoader` reads fresh from the `.:/app` mount; agent.py code changes also need the restart).
...
| `interview_runtime` | In-process helpers the engine calls directly (post-Phase-3 merge). `build_session_config` walks session → assignment → candidate → job → stage → bank → snapshot → questions → ancestry-walked company profile to build the engine's `SessionConfig`, and resolves the per-job `interview_engine_version` (?? global default) onto it (the v1↔v2 cutover point). `record_session_result` atomically updates the session row gated on `state='active'`, idempotent on retry, writes an audit row; the v2 `SessionResult` carries `coverage_summary`. Both run on a bypass-RLS session with explicit `tenant_id` filter on every query (RLS-only defense layer; no HTTP router, no engine-dispatch JWT — those were retired in migration `0025`). The wire-format Pydantic models (`SessionConfig`, `SessionResult`, `KnockoutFailure`) live here for the engine to import. |
...
### Current State
The initial schema (6 tables, first-cut RLS policies, system role seeds, and the Supabase auth hook) lives in `backend/supabase/migrations/20260405000000_initial_schema.sql`. Every incremental change since Phase 2A has been an Alembic migration in `migrations/versions/`. Current head: `0046_reset_emptied_banks`.
...
- `0042_question_difficulty` — Adds per-question `difficulty` to `stage_questions` (used by the v2 brain to calibrate grading strictness; the bank-generator emits it).
...
- `0044_interview_engine_version` — **Phase 3D.engine-v2.** Adds nullable `job_postings.interview_engine_version` (`'v1'`/`'v2'`); NULL inherits the global `INTERVIEW_ENGINE_DEFAULT_VERSION`. This is the per-job v1↔v2 cutover switch.
- `0045_bank_spoken_fields` — **engine-v2 bank (M2).** Adds `stage_questions.primary_signal` (nullable) and switches the `question_kind` CHECK to the v2 taxonomy. Dev-mode clear-and-regenerate (it `DELETE`s `stage_questions`, so banks are regenerated).
