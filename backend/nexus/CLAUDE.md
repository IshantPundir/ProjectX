...
- **Phase 3D.engine — the interview engine.** `app/modules/interview_engine/` is the sole interview engine, a structured **three-tier conversation/control split**. Its `agent.py` is the LiveKit entrypoint (it also carries the `server`/`prewarm`/`entrypoint` worker bootstrap); the entrypoint builds the `SessionConfig` and runs the engine unconditionally.
  ...
  - Supporting: `turn_taking/{floor,eou,pacing}.py` (livekit-free floor-control substrate), `directive.py` (closed act/move vocabulary + `is_terminal`), `event_log/` + `result_builder.py` (audit envelope + `SessionResult` with `coverage_summary`).
  - **Prompts** live under `prompts/v3/engine/` (`brain.system.txt`, `triage.system.txt`, `mouth/*`), read fresh per-session by `PromptLoader`. **Real-API prompt-evals** under the engine's `prompt_evals/` test subtree (`@prompt_quality`, opt-in: `pytest -m prompt_quality`) — the standing rule is to validate prompt behavior against the real OpenAI endpoint, not just mocks.
