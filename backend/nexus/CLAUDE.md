...
- **Schema management:** Supabase SQL for the initial cut + Supabase-managed objects (auth hook, `supabase_auth_admin` grants); Alembic for every incremental change after that. `migrations/versions/` head is `0062_report_share_tokens` (recent: `0048` drops the engine-selection flag → `0049` engine heartbeat → `0050` session recording columns → `0051` proctoring analysis → `0052` timeline thumbnails → `0053` session reels → … → `0059` drops the knockout columns → `0060` session reference photo → `0061` report shares → `0062` report-share tokens for the public recordings link).
- **Task queue:** Dramatiq + Redis. Broker setup is shared via `app/brokers.py`. Queues today: default + the per-task JD/bank queues (`jd_extraction`/`jd_enrichment`/`jd_reenrichment`/`question_bank_generation`), `report_scoring` (post-session report), `ats_poll` (manual ATS sync) — all run by the lean `nexus-worker`; `reel` + `vision` run in the dedicated `nexus-vision-worker`; `report_share` (render + email the report PDF) runs in the dedicated **`nexus-pdf-worker`** (Chromium/Playwright image). The lean `nexus-worker` is queue-restricted via an explicit `-Q` list so it never consumes the Chromium/GPU queues. Notification dispatch still runs via FastAPI `BackgroundTasks` (short, non-retryable).
...
- **Phase 3D.reporting — done.** `app/modules/reporting/` compiles the post-session report: a 3-layer hybrid that consumes the engine's append-only **`SessionEvidence`** (`sessions.session_evidence_json`) — per-question grade vs the bank rubric (`scoring/question_grade.py`, prompts `prompts/v4/report_scorer/`) → roll-up to per-(primary-)signal → LLM narrative. Verdict-driven fit-score (Overall/Technical/Behavioral/Communication ints → tier → verdict; state×texture bluff-aware; fit-ceiling-capped + ±5 holistic; Borderline always human-held). `session_reports` table (migration `0047`, UNIQUE on `session_id`); Dramatiq actor `score_session_report` (queue `report_scoring`) enqueued when the engine persists `SessionEvidence` (`record_session_evidence`); `/api/reports/*` (hub index, report view, presigned recording playback, proctoring view, human-decision write). Prompts under `prompts/v4/report_scorer/`. Spec: `docs/superpowers/specs/2026-05-28-verdict-driven-fit-score-design.md`.
  - **Report sharing (PDF) — done.** `POST /api/reports/session/{id}/share` (recruiter "Share" → recipient email) writes a tenant-scoped `report_shares` row and enqueues `share_report_pdf` (queue `report_share`, runs in `nexus-pdf-worker`). The actor renders a **client-curated** PDF of the existing `ReportRead` via a backend Jinja print template + **headless Chromium/Playwright** (`app/modules/reporting/pdf/`: `context.py` pure helpers, `render.py` lazy-Playwright + embedded BinQle logo, `templates/report.html.j2`), uploads it to R2 (`report-shares/{tenant}/{share_id}.pdf`), and emails it as an attachment (`notifications.send_email(..., attachments=...)`). Idempotent (`status=="sent"` short-circuit); recipient never travels in broker args (loaded from the row); audit event `session_report.shared`; recipient masked in logs. The PDF intentionally omits internal forensics (signal-audit, proctoring, methodology, human-decision). Coded verdict stamp (APPROVED/BORDERLINE/REJECTED). Spec: `docs/superpowers/specs/2026-06-16-report-sharing-pdf-design.md`; plan: `docs/superpowers/plans/2026-06-16-report-sharing-pdf.md`. **The render code runs in the long-lived `nexus-pdf-worker` (no hot-reload) — restart it after editing `reporting/pdf/` or the share actor.**
    - **Public recordings share — done.** The PDF's "See full session recording" button links to the candidate session app (`frontend/session`)'s public `/recordings/<token>` page (moved there from `frontend/app` on 2026-06-29 so the link works over the session ngrok tunnel in LAN mode, not just same-WiFi — link base is `candidate_session_base_url`, not the recruiter app). Spec: `docs/superpowers/specs/2026-06-29-public-recordings-share-to-session-design.md`. (Replaced the old `/shared-session/coming-soon` stub.) The actor mints a 256-bit opaque token (`reporting/share_tokens.py`), stores only its keyed HMAC hash + 1-year expiry on the `report_shares` row, and embeds the plaintext in the link. The unauthenticated `GET /api/public/recordings/{token}` (`reporting/public_router.py`, allowlisted in `middleware/auth.py::_PUBLIC_PREFIXES`) resolves the token on a bypass-RLS session → validates (revoked/expired) → tenant-scopes every read → returns one `PublicRecordingsEnvelope` (report + recording + proctoring + reel, freshly presigned). Uniform 404 on any bad token (no oracle). Revoke via `POST /api/reports/session/{id}/shares/{share_id}/revoke` (reports.view; backend-only, no UI yet). Spec: `docs/superpowers/specs/2026-06-16-public-recordings-share-design.md`; plan: `docs/superpowers/plans/2026-06-16-public-recordings-share.md`.
...
│       │   ├── timing.py         ← maps engine session-relative ms → recording video ms via a recording offset (meta.started_at − recording_started_at); clip spans come from the word-timed SessionEvidence transcript
...
├── migrations/                  ← Alembic — head is `0062_report_share_tokens`
│   └── versions/
├── tests/
...
### Current State
The initial schema (6 tables, first-cut RLS policies, system role seeds, and the Supabase auth hook) lives in `backend/supabase/migrations/20260405000000_initial_schema.sql`. Every incremental change since Phase 2A has been an Alembic migration in `migrations/versions/`. Current head: `0062_report_share_tokens`.
...
- `0060_session_reference_photo` — Adds `sessions.reference_photo_key` + `reference_photo_captured_at` (the candidate reference photo captured on Start; used as the report/PDF thumbnail).
- `0061_report_shares` — Adds the `report_shares` table (emailed PDF shares of a session report: recipient, status `pending|rendering|sent|failed`, `pdf_r2_key`, requester, timestamps). Canonical RLS pair; registered in `_TENANT_SCOPED_TABLES`. Powers the recruiter "Share" action → `share_report_pdf` actor on the `report_share` queue.
- `0062_report_share_tokens` — Adds the public-recordings capability-link columns to `report_shares` (`share_token_hash` + partial unique index, `share_expires_at`, `revoked_at`, `last_viewed_at`, `view_count`). The PDF's "See full session recording" button now links to `/recordings/<token>`; the token is stored only as a keyed HMAC hash.
