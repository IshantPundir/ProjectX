...
if TYPE_CHECKING:
    # Forward-declared so we don't pay the import cost from FastAPI workers
    # that never run a session — only the engine actually instantiates these.
...
def build_stt_plugin() -> "STT":
    ...
    return deepgram.STT(model=ai_config.stt_model, language=ai_config.stt_language)
