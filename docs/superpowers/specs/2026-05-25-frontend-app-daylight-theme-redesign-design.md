...
- **Page-polish — DONE** (plan `docs/superpowers/plans/2026-05-25-frontend-app-daylight-page-polish.md`; commits `40669529`..`8003a18b`). Swept every route-level hardcoded color to the Iris palette: pipeline stage journey + connectors, kanban avatar set + tracker job-type bars + status pills, JD editor, dashboard attention-card accents, org-units `detail.css` teal gradient + canvas/pressure badges, integrations/tabs, and the `SignalsPanel` `.px-dot` aria-hidden. Verified: zero old-palette hexes remain across `app/`+`components/`. (Node-type colors via `unit-type-style.tsx` were already Iris — they resolve through the remapped `--c-*` ramps.)
- **Genuinely future (not old-color leftovers):** chart palette wiring on the Reports module *when it's built* (still a placeholder; `--chart-*` tokens are ready), and optional `-fill` pastel *category fills* on kanban columns / stat cards as an enhancement. **Op note:** don't run `next build` against the live dev `.next` (collides with Turbopack `npm run dev` → `ChunkLoadError`); verify with `type-check`/`lint`.
