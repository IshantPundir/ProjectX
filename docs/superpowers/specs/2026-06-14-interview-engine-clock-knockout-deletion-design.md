...
|---|---|---|
| **Clock** | `BudgetPhase`, `BudgetConfig`, `compute_budget_phase`, `budget_config_from_ai_config`; all budget/time params in `resolve_next`; `BrainTurnInput.budget_phase` + its prompt render block; driver `_time_remaining_s` / `_budget_cfg` / time wiring + `_BrainAdapter.time_remaining_s`; `engine_close_reserve_s` / `engine_winding_down_s` (AIConfig + `config.py`); winding-down language in `brain.system.txt` | `SessionMeta.duration_s` (elapsed wall-clock) + `SessionMeta.time_budget_s` (planned duration — durable audit artifact, not runtime clock), `started_at` / `ended_at`; the anti-stall counter (`_stall_*`, unrelated) |
